...
Firstly, we need proprocess data a bit to fix factor variable `level` and create a new variable `item` for fulure CFA.
...
```{r}
NASA_TLX %>% 
  pivot_wider(id_cols = id, names_from = item, values_from = score) -> NASA_TLX_w
names(NASA_TLX_w)
```
