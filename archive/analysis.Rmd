...
MR |> 
  ...
  left_join(exp_time) -> MR_exp_time
...
```{r}
aov(acc ~ group * level, MR_exp_time) |> summary()
```
...
MS |> 
  ...
  left_join(exp_time) -> MS_exp_time
...
```{r}
aov(acc ~ group * level, MS_exp_time) |> summary()
t.test(MS_exp_time$acc ~ MS_exp_time$group)
```
...
```{r}
aov(acc ~ group * level, ST_exp_time) |> summary()
```
...
ST |> 
  group_by(id, level) |> 
  summarise(acc = mean(is_correct)) |> 
  left_join(exp_time) -> ST_exp_time
...
```{r}
exp_time_info <- tibble()

for (i in 1:length(files)) {
  tibble(id = str_extract_all(files[i], "\\d{3}[:upper:]{4}") |> 
           unlist(),
         start = files[i] |> 
         ...
         end = file.info(files[i])$mtime |> 
           as_datetime(tz = "UTC")
  ) |> 
    bind_rows(exp_time_info) -> exp_time_info
}

exp_time_info |> 
  mutate(dur = abs(start - end)) -> exp_time_info
```
...
exp_time_info$dur |> min(na.rm = TRUE)
quantile(exp_time_info$dur[exp_time_info$dur<250], .75, na.rm = TRUE) + 1.5 * IQR(exp_time_info$dur[exp_time_info$dur<250], na.rm = TRUE)
```
