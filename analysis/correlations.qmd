...
```{r}
tibble(
  ME = c(1, .57, .58, .36, .76, .54, .84),
  PH = c(.57, 1, .5, .27, .58, .44, .70),
  TI = c(.58, .50, 1, .32, .66, .52, .67),
  PE = c(.36, .27, .32, 1, .4, .57, .46),
  EF = c(.76, .58, .66, .40, 1, .69, .84),
  FR = c(.54, .44, .52, .57, .69, 1, .70),
  OW = c(.84, .70, .67, .46, .84, .70, 1)
) %>% 
  as.matrix() -> original
row.names(original) <- row.names(pooled_matrix)

original - pooled_matrix -> diff
max(abs(diff))

max(abs(eigen(diff)$values))
...
psych::cortest.jennrich(R1 = original,
                        R2 = pooled_matrix,
                        n1 = 6,
                        n2 = 82+69)
```
